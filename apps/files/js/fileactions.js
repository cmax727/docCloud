FileActions={
    ...
    display:function(parent, filename, type){
        ...
        if(actions['Delete'] && (type!='dir' || filename != 'Shared')){ // no delete action for the 'Shared' dir
            ...
            element.click(function(event){
                OC.dialogs.confirm(t('contacts', 'Are you sure you want to delete this?'), t('contacts', 'Warning'), function(answer) {
                    if ( true && ! answer)return ;    
                event.stopPropagation();
                event.preventDefault();
                var action=actions[element.data('action')];
                var currentFile=FileActions.getCurrentFile();
                FileActions.hide();
                action(currentFile); 
                });
